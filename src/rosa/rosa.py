...
#这个AgentExecutor是Langchain的一个类，负责执行agent的逻辑，包括调用工具、处理模型输出等。
#下面的参数配置都具体可以参考https://reference.langchain.com/python/langchain-classic/agents/agent/AgentExecutor官方文档
    def _get_executor(self, verbose: bool) -> AgentExecutor:
        ...
        executor = AgentExecutor(
            agent=self.__agent,#负责思考的大脑，就是我们刚刚创建的 agent
...
#调用Langchain的create_tool_calling_agent来创建一个agent，传入我们之前准备好的 llm、工具列表和 prompts。这个 agent 就是我们 ROSA 的核心，它会根据用户的输入和上下文来决定调用哪些工具，生成什么样的响应。
    def _get_agent(self):
...
################
    ...
    def _get_tools(
        ...
        return rosa_tools#先 new 一个 ROSATools(...)对象
                        #如果你额外传了工具，就加进去
                        #如果你额外传了工具包，也加进去
                        #最后返回这个对象
